...
type Adapter = { adapterId: string; repository: string; model: string; executionStatus: string; reason: string; contribution: string; ran: boolean; artifact?: string; statistics?: Record<string, unknown>; findingRecords?: Array<Record<string, unknown>> };
    ...
    <main className="drift-shell">
      ...
      <div className="workspace-grid">
        <aside className="side-rail">
          ...
          {result && <div className="adapter-sidebar"><p className="eyebrow">REPOSITORY DETECTIONS</p>{result.adapters.map((adapter) => <button key={adapter.adapterId} className={selectedAdapter?.adapterId === adapter.adapterId ? "adapter-link active" : "adapter-link"} onClick={() => { setSelectedAdapterId(adapter.adapterId); setActiveTab("evidence"); }}><span><strong>{topicFor(adapter.adapterId)}</strong><small>{adapter.executionStatus}</small></span><b>{adapter.findingRecords?.length ?? visibleFindings.filter((finding) => finding.source?.repository?.toLowerCase().includes(adapter.repository.toLowerCase())).length}</b></button>)}</div>}
