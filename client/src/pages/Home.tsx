...
const demoSpecs: Record<string, { input: string; detects: string; mode: string; ready: boolean }> = {
  mustatil: { input: "GeoTIFF / satellite image", detects: "GIS workspace review; not video ML", mode: "Input-gated", ready: false },
  "foundation-models-archaeology": { input: "Satellite / LiDAR / notebook input", detects: "Archaeological feature analysis", mode: "Input-gated", ready: false },
  adaf: { input: "ALS / LiDAR GeoTIFF", detects: "Archaeological features in terrain", mode: "Input-gated", ready: false },
  arran: { input: "Arran benchmark dataset", detects: "Benchmark scoring, not video detection", mode: "Dataset", ready: false },
  "simulated-training-data": { input: "DEM + stream vectors", detects: "Synthetic terrain/training outputs", mode: "Input-gated", ready: false },
  "uav-thermal-person-geolocation": { input: "Thermal video + DJI SRT", detects: "People in thermal imagery + GPS projection", mode: "Needs thermal/SRT", ready: false },
  "drone-tracker": { input: "RGB drone video + checkpoint", detects: "Objects / people tracking", mode: "Needs checkpoint", ready: false },
  "aerial-thermal-detection": { input: "Thermal video", detects: "Thermal people/objects with YOLO/RT-DETR", mode: "Needs thermal", ready: false },
  "aerial-thermal-sar-detection-demo": { input: "Thermal image/video", detects: "Thermal SAR person detection", mode: "Needs thermal", ready: false },
  "rgbt-fusion-drone-sar": { input: "Synchronized RGB + thermal", detects: "RGB-T fused objects", mode: "Needs paired inputs", ready: false },
  "ros2-disaster-robot-sim": { input: "ROS2 disaster-world simulation", detects: "Robot simulation state; not video ML", mode: "Simulator", ready: false },
  "drone-control-monitoring-system": { input: "MAVLink / telemetry log", detects: "Flight/telemetry events; not video ML", mode: "Telemetry", ready: false },
};
    ...
    <main className="drift-shell">
      ...
      <div className="workspace-grid">
        ...
        <section className="main-stage">
          ...
          <section className="demo-lab"><div className="gallery-heading"><div><p className="eyebrow">DEMO LAB / 12 PIPELINES</p><h3>What each repository can actually detect</h3></div><span>REAL INPUT CONTRACTS · NO FABRICATED FINDINGS</span></div><div className="demo-grid">{modules.map((module) => { const spec = demoSpecs[module.id]; return <article className="demo-card" key={module.id}><div className="demo-card-top"><strong>{module.label}</strong><span className={spec.ready ? "demo-ready" : "demo-gated"}>{spec.mode}</span></div><p><b>Input:</b> {spec.input}</p><p><b>Output:</b> {spec.detects}</p><button className="outline-button" onClick={() => { setTdmPipelineId(module.id); setTdmMode(true); setActiveTab("pipeline"); }}>Select pipeline</button></article>; })}</div></section>
