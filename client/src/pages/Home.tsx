...
export default function Home() {
  ...
  const runMission = async () => {
    ...
    try {
      ...
      const queued = await queueMission({ executionMode, videoUri, fileName: tdmMode ? (demoVideoMode ? "delhi-collapse-demo.mp4" : "tdm-rgb-video.mp4") : selectedFile!.name, thermalVideoUri: tdmMode ? tdmInput.thermalVideoUri : thermalMode ? videoUri : undefined, ...tdmInput, enabledModules: modules.filter((module) => enabled[module.id]).map((module) => module.id) });
