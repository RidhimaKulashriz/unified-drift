    ...
    <main className="drift-shell">
      ...
      <div className="workspace-grid">
        ...
        <section className="main-stage">
          ...
          <section className="detection-catalog"><div className="gallery-heading"><div><p className="eyebrow">12 / 12 SYSTEMS COVERED</p><h3>12 SYSTEMS — WHAT THEY DETECT / DO</h3></div><span>EXACTLY 12 PANELS · PROVENANCE PRESERVED</span></div><div className="detection-panel-grid">{detectionPanels.map((panel, index) => <article className="detection-panel" key={panel.id}><div className="detection-panel-media">{panel.image ? <img src={panel.image} alt={`${panel.task} annotated result`} /> : <div className="visual-empty"><Radar size={22} /><span>EXPECTED TASK / TARGET</span><small>{panel.evidence}</small></div>}</div><div className="detection-panel-body"><span className="panel-index">{String(index + 1).padStart(2, "0")}</span><h4>{panel.task}</h4><p><b>What it detects / does:</b> {panel.detects}</p><p><b>Detection/task category:</b> {panel.output}</p><p><b>Example target/result:</b> {panel.image ? "Official annotated upstream example" : "Real input/task explanation; no prediction fabricated"}</p><small className="demo-caption">{panel.image ? "Official upstream result visual; not this run." : panel.evidence}</small><div className="demo-card-actions"><span className="outline-button">View detection example</span><span className="demo-source">Source attribution preserved internally</span></div></div></article>)}</div></section>
