    ...
    <main className="drift-shell">
      ...
      <div className="workspace-grid">
        ...
        <section className="main-stage">
          ...
          <section className="detection-catalog"><div className="gallery-heading"><div><p className="eyebrow">WHAT EACH REPOSITORY DETECTS</p><h3>12 distinct detection tasks</h3></div><span>IMAGE EVIDENCE IS LABELED BY PROVENANCE</span></div><div className="detection-panel-grid">{detectionPanels.map((panel, index) => <article className="detection-panel" key={panel.id}><div className="detection-panel-media">{panel.image ? <img src={panel.image} alt={`${panel.task} annotated result`} /> : <div className="visual-empty"><Radar size={22} /><span>No verified image artifact</span><small>{panel.evidence}</small></div>}</div><div className="detection-panel-body"><span className="panel-index">{String(index + 1).padStart(2, "0")}</span><h4>{panel.task}</h4><p><b>Detects:</b> {panel.detects}</p><p><b>Returns:</b> {panel.output}</p><small className="demo-caption">{panel.image ? "Official upstream result visual; not this run." : panel.evidence}</small><a className="demo-source" href={panel.source} target="_blank" rel="noreferrer">repository evidence ↗</a></div></article>)}</div></section>
